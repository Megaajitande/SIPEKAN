...
export default function Tabelwaktukunjungan({ waktu_kunjungan }) {   
    const [editwaktu, setEditwaktu] = useRecoilState(editData);
    const [showForm, setShowForm] = useRecoilState(form);
    const [lihatwaktu, setlihatwaktu] = useState(false);
    ...
    const hapus = (id) => {
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    Inertia.post('/hapusdatawaktu', formData);
                }
            });
    ...
    return (
            ...
            <Grid
                server={{
                    url: "http://localhost:8000/api/waktukunjungan",
                    then: (data) =>
                        data.map((waktu_kunjungan, index) => [
                            index + 1,
